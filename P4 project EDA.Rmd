...
ProsperScore is supposed to have values ranging from 1-10. But it also seem to have
11. 
...
Explore other variables of interest that might define the Loan Status:
...
ggplot(aes(x=TotalInquiries), data =  loan) + 
...
* Most Prosper Score metric is between 3 to 9.

* Most Credit Score is between 650 and 750. 

* Most frequent Income Range is $25k - 75k, and 

* Most Debt to Income Ratio is between .1 to .3

* Most inquiries seems to be between 0 to 6.
...
* Term: There are 3 values for Terms (12,36, and 60). 36 month seems to be most common.
...
"BorrowerAPR", "ProsperRating..Alpha.", "EmploymentStatus", "EmploymentStatusDuration",
"IsBorrowerHomeowner", etc.
...
ggplot(aes(x=LoanStatus_mod ,y=DelinquenciesLast7Years), data = subset(loan, DelinquenciesLast7Years>0)) +
  geom_boxplot() +
...
In general, mean credit scores for people with bad loans are lower compared to 
...
For employed people, Income range and mean Credit Score seems to be good indicator
of Loan Status. For example, people with Income Range of $1-24k with 'Bad' loan status
have average Credit score of ~650. Whereas people in same Income Range with 
'Completed' Loan Status have mean Credit Score of ~670. This trend is seen for all
Income Ranges.
...
```{r echo=FALSE, message=FALSE, warning=FALSE, Multivariate_Plots2a, fig.height=8,fig.width=8}
...
When we add InquiriesLast6Months into Credit Score and Income Range, 
it shows, on average, there are more Inquiries in Last 6 Months for Bad Loans, compared to
other loan types.

Completed loans have lower number of Inquiries in Last 6 Months, but they are still 
larger than Current loans (esp. in lower Credit Score ranges). This might be
because of the fact that completed loans includes past loans where investors were
more willing to take larger risks (and therefore reaped larger rewards). But the investors
for current loans are less willing to take risks.


Study relationship between Income Range and Loan Status, by Income-Vertifiable.
...
Loan Status values are uniformly spread out across range of Debt to income Ratio values,
for different Income Range values.
...
Another interesting observation is that alonst all Current loans have DebtToIncomeRatio
below 1, peaking around .7. What can be the reason for this? Maybe this the threshold where 
risk and return are optimal?
...
inquiries are significantly higher for Bad Loans compared to Completed Loans.
...
to Loan Status. Bad Loans have higher mean number of Inquiries compared to the good ones,
...
### Plot Two
```{r echo=FALSE,  message=FALSE, warning=FALSE,Plot_Two, fig.height=6,fig.width=8}
...
The mean credit score for 'current' loans are very high. This might be because
people have started avoiding loan listing that have attributes which could potentially
lead to the loan being defaulted - in other words, investors taking less risks with
the loans.
...
between Completed loan and Bad Loans. Current loans were also studied. 
...
More work needs to be done to identify the strategies to generate most
